...
function DataInsights() {
  ...
  const handleBulkFileUpload = (e) => {
    ...
    if (files.length > 10) {
      showToast('You can upload a maximum of 10 files at a time.', 'success');
  ...
  const handleGetInsights = async () => {
    if (!uploadedFile) {
      showToast('Please upload a file first.', 'success');
      return;
    }
    ...
    try {
      ...
      if (uploadData.message === 'File already exists') {
        showToast('The file already exists on the server. It will not be uploaded again.', 'success');
      } else {
  ...
  if (bulkFiles.length === 0) {
    showToast('Please select up to 10 PDF files for bulk upload.', 'success');
    return;
  }
  ...
  try {
    ...
    showToast('An error occurred while processing your files.', 'error');
