...
function Header({ onProcessClick }) {
  ...
  const handleLandscapeClick = async () => {
    ...
    try {
      const response = await fetch(API_CONFIG.CHROME_HISTORY + '?user_id=' + firstName);
  ...
  const handleTestConnection = async () => {
    ...
    try {
      const response = await fetch(API_CONFIG.TEST_CONNECTION, {
        method: 'POST',
  ...
  const handleSystemClick = async () => {
    ...
    try {
      const toolsRes = await fetch(API_CONFIG.GET_TOOLS_LANDSCAPE);
      ...
      const agentsRes = await fetch(API_CONFIG.RECOMMEND_AGENTS, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
